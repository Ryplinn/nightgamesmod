...
public class PressurePointed extends DurationStatus {
    ...
    public String describe(Combat c) {
        return Formatter.format("{self:SUBJECT-ACTION:are} clenching {self:possessive} lower muscles to prevent {self:reflective} from cumming instantly.", affected, c.getOpponent(affected));
    }
    ...
    public void tick(Combat c) {
        if (getDuration() <= 0) {
            ...
            c.write(Formatter.format("The feeling in {self:name-possessive} crotch finally passes and {self:pronoun} can finally relax; whatever {other:SUBJECT} did to {self:direct-object} seems to have subsided.", affected,
                            c.getOpponent(affected)));
        } else if (!affected.canRespond()) {
            c.write(Formatter.format("<b>Without {self:NAME-DO} actively holding it in, the artifically induced orgasm rips through {self:possessive} body.</b>", affected, c.getOpponent(affected)));
            affected.removelist.add(this);
