...
public class Collar extends Skill {
    ...
    public boolean resolve(Combat c, Character target) {
        if (target.is(Stsflag.collared)) {
            ...
            c.write(getSelf(), Formatter.format("{self:SUBJECT-ACTION:replace|replaces} the batteries"
                            + " of {other:name-possessive} collar, so it can keep going for longer.",
                            getSelf(), target));
        } else {
            c.write(getSelf(), Formatter.format("Able to take {self:possessive} time - given"
