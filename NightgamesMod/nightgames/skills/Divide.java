...
public class Divide extends Skill {
    ...
    public boolean requirements(Combat c, Character user, Character target) {
        return user.has(Trait.BinaryFission) && user.has(Trait.slime);
    }
    ...
    public static Pet makeClone(Combat c, Character self) {
        ...
        pet.getSelf().getSkills().add(new SlimeCloneParasite());
        return pet;
    ...
    public boolean resolve(Combat c, Character user, Character target) {
        ...
        c.write(user, formatMessage(target, user));
    ...
    private String formatMessage(Character target, Character user) {
        if (user.human()) {
            return Formatter.format("You focus your attention on your slimy constitution and force yourself apart. "
                            + "The force of effort almost makes you black out, but when you finally raise your head, you are face to face with your own clone!", user, target);
        } else {
