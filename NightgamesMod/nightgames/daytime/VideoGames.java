...
public class VideoGames extends Activity {
        ...
        if (choice.equals("Start")) {
            ...
            if (paid) {
                if (Random.random(3) == 0) {
                    ...
                    getPlayer().gainXPPure(50);
                    getPlayer().spendXP();
                    getPlayer().spendLevels(null);
                }
            }
        }
    }
    ...
    private void showScene(Scene chosen) {
        switch (chosen) {
            ...
            case mara1:
                ...
                getPlayer().gainAffection(GameState.getGameState().characterPool.getNPC("Mara"), 1);
                GameState.getGameState().characterPool.getNPC("Mara").gainAffection(getPlayer(), 1);
            ...
            case cassiemara:
                ...
                GameState.getGameState().characterPool.getNPC("Mara").gainAffection(GameState.getGameState().characterPool.getNPC("Cassie"), 1);
                GameState.getGameState().characterPool.getNPC("Cassie").gainAffection(GameState.getGameState().characterPool.getNPC("Mara"), 1);
                break;
            case caroline1:
                GUI.gui.message(
    ...
    private Scene pickScene() {
        ...
        if (getPlayer().getAffection(GameState.getGameState().characterPool.getNPC("Cassie")) >= 5) {
            available.add(Scene.cassie1);
        }
        if (getPlayer().getAffection(GameState.getGameState().characterPool.getNPC("Cassie")) >= 10 && Flag.checkFlag(Flag.Cassievg1)) {
            available.add(Scene.cassie2);
        }
        if (getPlayer().getAffection(GameState.getGameState().characterPool.getNPC("Mara")) >= 5) {
            available.add(Scene.mara1);
        }
        if (getPlayer().getAffection(GameState.getGameState().characterPool.getNPC("Mara")) >= 10 && Flag.checkFlag(Flag.Maravg1)) {
            available.add(Scene.mara2);
        }
        if (GameState.getGameState().characterPool.getNPC("Cassie").getAffection(GameState.getGameState().characterPool.getNPC("Mara")) >= 10 && Flag.checkFlag(Flag.Cassievg2)
                        && Flag.checkFlag(Flag.maravg2)) {
            available.add(Scene.cassiemara);
        }
        if (getPlayer().getAffection(GameState.getGameState().characterPool.getNPC("Angel")) >= 10) {
