...
public class Meditation extends Activity {
    ...
    public void visit(String choice, int page, List<LabeledValue<String>> nextChoices, ActivityInstance instance) {
        ...
        if (Flag.checkFlag(Flag.dojo) && choice.equals("Start")) {
            ...
            if (getPlayer().money >= 1000 * (getPlayer().getPure(Attribute.ki) + 1)) {
                ...
                getPlayer().money -= 1000 * (getPlayer().getPure(Attribute.ki) + 1);
                getPlayer().modAttributeDontSaveData(Attribute.ki, 1);
                Flag.flag("Trained" + Attribute.ki.name());
                acted = true;
                getPlayer().gainIfAbsent("gi");
                choose("Leave", nextChoices);
            } else {
            ...
            if (Random.random(100) >= 50) {
                ...
                if (getPlayer().getPure(Attribute.perception) < 9) {
                    getPlayer().modAttributeDontSaveData(Attribute.perception, 1);
                }
            } else {
                GUI.gui.message("Suzume instructs you to sit in the middle of the dojo and close your eyes. <i>\"I'm going to count down from ten. With each number, you will feel your "
                                + "mind opening and you will be more receptive to my words. When I reach zero, imagine your mind as an empty vessel, ready to take in everything around you.\"</i> As she "
            ...
            if (Random.random(100) >= 50) {
                ...
                if (getPlayer().getPure(Attribute.perception) > 1) {
                    getPlayer().modAttributeDontSaveData(Attribute.perception, -1);
                }
            } else {
                GUI.gui.message("Suzume has you lie down on the cold, somewhat uncomfortable floor. <i>\"Now please close your eyes while I count down from ten. I want you to imagine a set of stairs and "
